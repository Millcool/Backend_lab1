...
class TestCompf:
    ...
    def test_parentheses1(self):
        compiler = Compf()
        compiler.val = 'fgh'
        compiler.try_add_value('f')
        expected_data = ['f']
        assert compiler.data == expected_data
        #assert self.c.compile("(ab)") == "a b"
    ...
    def test_parentheses4(self):
        assert self.c.compile("(((((((ant+b)*((a+b)))))))") == \
               "a n t b + a b + *"

    # Тесты на более сложные выражения
    def test_expressions1(self):
        assert self.c.compile("(a+b)*c+(d-e)/f") == \
               "a b + c * d e - f / +"
